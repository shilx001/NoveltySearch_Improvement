...
# run the GA with novelty search, with hindsight experience replay to periodically inject to the population
# 2020-2-8
class HP:
...
class Policy:
    ...
    def get_fitness(self):
        ...
        for step in range(400):
            ...
            obs = next_obs
            total_reward += reward
            if done or step is 399:
                break
        if len(self.hp.archive) > 10:
...
for episode in range(50):
    ...
    position = []
    for p in population:
        n, r, last_position = Policy(hp, params=p).get_fitness()  # 此时应该存储好trajectory
        novelty.append(n)
        reward.append(r)
        position.append(last_position)
    fitness = novelty
    #针对每个trajectory, 先做一般的experience replay,再做hindsight experience replay，存储到ddpg的replay buffer里。
